...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const modulesSlider = new _modules_sliders_slider_mini__WEBPACK_IMPORTED_MODULE_1__["default"]({
  ...
  const feedSlider = new _modules_sliders_slider_mini__WEBPACK_IMPORTED_MODULE_1__["default"]({
  ...
  new _modules_videoPlayer__WEBPACK_IMPORTED_MODULE_2__["default"]('.module__video-item .play', '.overlay').render();
...
class Form {
  ...
  bindForm() {
    this.forms.forEach(item => {
      item.addEventListener('submit', e => {
        ...
        this.postData(this.path.server, formData).then(data => {
          console.log(data);
  ...
  initPhoneMask() {
    ...
    function createMask(event) {
      ...
      this.value = matrix.replace(/./g, s => /[_\d]/.test(s) && i < val.length ? val.charAt(i++) : i >= val.length ? '' : s);
  ...
  bindClose() {
    const closePlayer = () => {
      if (this.player) {
        ...
        this.overlay.querySelector('#iframe').remove();
        const block = document.createElement('div');
        this.overlay.querySelector('.video').prepend(block);
        block.id = 'iframe';
      }
    };
